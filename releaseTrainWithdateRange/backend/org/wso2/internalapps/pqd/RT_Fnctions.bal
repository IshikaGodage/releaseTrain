...
function getAllReleases(int startFormat, int endFormat)(json){
    ...
    int currentFormat = currentTime.time;
    string currentDate = time:format(currentTime, "yyyy-MM-dd'T'HH:mm:ssz");

    time:Time startTime ={time:startFormat, zone:{zoneId:"Asia/Colombo",zoneOffset:19800}};
    string startDate = time:format(startTime,"yyyy-MM-dd'T'HH:mm:ssz");

    time:Time endTime ={time:endFormat, zone:{zoneId:"Asia/Colombo",zoneOffset:19800}};
    string endDate = time:format(endTime,"yyyy-MM-dd'T'HH:mm:ssz");

    system:println(currentTime);

    system:println(currentDate);
    system:println(startDate);
    system:println(endDate);

    system:println(currentFormat);
    ...
    sql:Parameter[] params10 = [];

    sql:Parameter emptyString = {sqlType:"varchar", value:""};

    sql:Parameter startTimeStamp = {sqlType:"varchar", value:startDate};
    sql:Parameter endTimeStamp = {sqlType:"varchar", value:endDate};
    sql:Parameter currentTimeStamp = {sqlType:"varchar", value:currentDate};



    if(startFormat<currentFormat && endFormat>currentFormat){
        params1=[emptyString, currentTimeStamp, endTimeStamp];
        params10=[startTimeStamp,currentTimeStamp];
    }else if(startFormat<currentFormat && endFormat<currentFormat){
        params1=[emptyString, currentTimeStamp, currentTimeStamp];
        params10=[startTimeStamp,endTimeStamp];
    }else if(startFormat>currentFormat && endFormat>currentFormat){
        params1=[emptyString, startTimeStamp, endTimeStamp];
        params10=[currentTimeStamp, currentTimeStamp];
    }
    ...
    datatable dtGitHubReleaseDates = rmDB.select(GET_ALL_GITHUB_RELEASE_DATES, params10);
...
function getReleasesByProductArea (string productArea, int startFormat, int endFormat) (json) {
    ...
    if(startFormat<currentFormat && endFormat>currentFormat){
        params1=[empty, mainProductArea, currentTimeStamp, endTimeStamp];
        params10=[mainProductArea, startTimeStamp,currentTimeStamp];
    }else if(startFormat<currentFormat && endFormat<currentFormat){
        params1=[empty, mainProductArea, currentTimeStamp, currentTimeStamp];
        params10=[mainProductArea, startTimeStamp,endTimeStamp];
    }else if(startFormat>currentFormat && endFormat>currentFormat){
        params1=[empty, mainProductArea, startTimeStamp, endTimeStamp];
        params10=[mainProductArea, currentTimeStamp, currentTimeStamp];
    }
    ...
    //GitHub release details
        ...
        datatable dtGitHubReleaseDates = rmDB.select(GET_ALL_GITHUB_RELEASE_DATES_BY_PRODUCT_AREA, params10);
        var gitHubReleaseDatesJson, err = <json>dtGitHubReleaseDates;
        logger:debug(gitHubReleaseDatesJson);
        datatables:close(dtGitHubReleaseDates);
